...
local max_distance = ui.new_slider("lua", "b", "Max aimbot distance", 10, 2000, 1000, true, "ft", 1)
            ...
            if model_name and model_name == "models/props/coop_cementplant/exloding_barrel/exploding_barrel.mdl" then
                ...
                if x then
                    ...
                    if wx then 
                        ...
                        text(wx, wy + 15, r, g, b, a, "c", 0, distance_ft .. "FT")
                        
                        -- 如果被锁定，显示瞄准偏移高度（转换为英尺）
                        if ent == cur_target then
                            local height_offset = get(aim_height_offset) or 20
                            local height_offset_ft = units_to_feet(height_offset)
                            text(wx, wy + 30, locked_r, locked_g, locked_b, locked_a, "c", 0, "+" .. height_offset_ft .. "FT")
                        end
                    end
                end
            end
