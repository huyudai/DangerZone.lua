...
-- 定义支持的近战武器列表（使用CS:GO实际的武器ID）
local supported_melee_weapons = {
    [75] = true,  -- 
    [76] = true,  -- 
    [78] = true,  --
    [69] = true,  -- 拳头 (Fists)
    [37] = true   -- 盾牌 (Shield) - 添加盾牌
}
...
-- 盾牌自瞄逻辑
local function shield_aim_logic(cmd)
    local local_player = get_local_player()
    ...
    if weapon_index ~= 37 then return end  -- 37是盾牌的武器ID
...
client.set_event_callback("setup_command", function(cmd)
    ...
    -- 盾牌自瞄逻辑
    if ui_get(punchbot_checkbox) and weapon_index == 37 then
        if ui_get(punchbot_hotkey) then
            shield_aim_logic(cmd)
        end
        return
    end
    
    -- 原有的近战武器自瞄逻辑
    if ui_get(punchbot_checkbox) and supported_melee_weapons[weapon_index] then
        local target_position = get_best_target()
