local remove_grass = ui.new_checkbox("lua", "a", "Remove DangerZone Grass")
local grass_state = false
local function grass_on_update()
	grass_state = false
end
ui.set_callback(remove_grass, grass_on_update)
client.set_event_callback("player_connect_full", grass_on_update)
local function grass_remove()
	...
	elseif map_name == "dz_county" then
		...
		local mats = materialsystem.find_materials("detail/")-----very bad shit
		for i = 1, #mats do
