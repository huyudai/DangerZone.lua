--Source:Aimware-DangerZone-Lua
--Last Updated 2023/8/15 1.1.5 (New Version)
--This version migration is incomplete and cannot be loaded
...
local esp_group_name = "DangerZone Elite ESP"
...
local espmaster = menu.add_checkbox(esp_group_name, "ESP Master Switch", true)
local showammo = menu.add_checkbox(esp_group_name, "Show Ammo", true)
local showdistance = menu.add_checkbox(esp_group_name, "Show Distance", true)
local showteamstatus = menu.add_checkbox(esp_group_name, "Show TeamStatus", true)
local showguntype = menu.add_checkbox(esp_group_name, "Show GunType", true)
local barrelmaster = menu.add_checkbox(esp_group_name, "Show Barrel ESP", true)
local remotebombmaster = menu.add_checkbox(esp_group_name, "Show RemoteBomb ESP", true)
local removegrassmaster = menu.add_checkbox(esp_group_name, "Remove Grass", true)

local line_group_name = "DangerZone Elite Line"
...
local linemaster = menu.add_checkbox(line_group_name, "Line Master Switch", true)
local dronemaster = menu.add_checkbox(line_group_name, "Drone Detector", true)
local boxmaster = menu.add_checkbox(line_group_name, "EndZone Detector/Line", true)
local guidemaster = menu.add_checkbox(line_group_name, "GuideLine(Very useful)", true)
local wpmaster = menu.add_checkbox(line_group_name, "Weapon Snapline", true)
local shieldarmormaster = menu.add_checkbox(line_group_name, "Shield/Armor Snapline", true)
local linesubmaster = menu.add_checkbox(line_group_name, "SubLine Master Switch", true)
local hsmaster = menu.add_checkbox(line_group_name, "Hostage Snapline", true)
local ammomaster = menu.add_checkbox(line_group_name, "Ammobox Snapline", true)
local endcirclemaster = menu.add_checkbox(line_group_name, "NormalBox Snapline", true)
local cashmaster = menu.add_checkbox(line_group_name, "Cash Snapline", true)
local healmaster = menu.add_checkbox(line_group_name, "Healthshot Snapline", true)
...
local pLocal = nil
local font = render.create_font("Microsoft Tai Le", 45, 1000);
local fontA = render.create_font("Microsoft Tai Le", 80, 1000);
local font1 = render.create_font("Verdana", 22, 400);
local screen = get_screen_size()
local screenCenterX, screenH = screen.x, screen.y
...
local function returnweaponstr(player)
	if player:is_player() and player:is_alive() then
		local recstr = ""
		for i = 0, 63 do
			local weapon = player:get_prop("m_hMyWeapons", i)
			if weapon:get_class_name() ~= nil then
...
local function returngunid(player)
	local active_weapon = player:get_active_weapon()
	if (active_weapon == nil) then
		return
	end
	return active_weapon:get_prop("m_iItemDefinitionIndex")
end

local function w2sreturnxy(object)
	local v2tres = render.world_to_screen(object)
	if v2tres == nil then return nil, nil end
	return v2tres.x, v2tres.y
end
...
callbacks.Register("CreateMove", function()
	if not espmaster:get() or not gui.GetValue("esp.master") or not isNeedW or not showteamstatus:get() or client.GetConVar("game_type") ~= "6" then
		Teamcalled = false
		return
	end
	playerdata = {}
	...
	if Enemies ~= nil then
		...
		for _, player in ipairs(Enemies) do
			if player:GetName() ~= "GOTV" and player_resource.get_prop("m_iPing", player:get_index()) ~= 0 then
				local teamstr = "team" .. player:get_prop("m_nSurvivalTeam")
				if player_resource.get_prop("m_bHasCommunicationAbuseMute", player:get_index()) == 1 and teamstr ~= "team-1" then
					abuseteam[teamstr] = true
				end
				if playerdata[teamstr] == nil then playerdata[teamstr] = {} end
				if player:IsAlive() then
					table.insert(playerdata[teamstr],
						{ player:get_index(), player:get_name(), player:get_render_origin(), player:is_alive() })
				else
					table.insert(playerdata[teamstr],
						{ player:get_index(), player:get_name(), player:get_render_origin(), player:is_alive() })
				end
...
local function drawEspHookESP(ctx)
	-- if not espmaster:get() or not gui.GetValue("esp.master") or not isNeedW then return end
	if not espmaster:get() or not isNeedW then return end

	local builder_entity = ctx.player
	local builder = ctx
	...
	if not builder_entity:is_player() or not builder_entity:is_alive() then return end
	if showammo:get() and plocallive then
		local current_weapon = builder_entity:get_active_weapon()
		if current_weapon:get_prop("m_iClip1") ~= nil then
			--builder:Color(0, 150, 240, 255)
			builder:add_flag(tostring(current_weapon:get_prop("m_iClip1") ..
				"/" .. current_weapon:get_prop("m_iPrimaryReserveAmmoCount")), color_t(0, 150, 240))
		end
	end
	local lpabs = builder_entity:get_render_origin()

	if showdistance:get() and plocallive then
		local Distance = math.floor((lpabs - pLocal:get_render_origin()):Length())
		if Distance ~= nil then
			builder:add_flag("D:" .. Distance, color_t(0, 150, 240))
		end
	end

	if showteamstatus:get() and Teamcalled then
		-- builder:Color(255, 255, 255, 255)
		local righttext = ""
		local teamstr = "team" .. builder_entity:get_prop("m_nSurvivalTeam")
		local ischeater = player_resource.get_prop("m_bHasCommunicationAbuseMute", builder_entity:get_index())
		...
		if righttext ~= "" then
			builder:add_flag(righttext, color_t(0, 150, 240))
		end
	end
	if showguntype:get() and plocallive then
		-- builder:Color(255, 0, 0, 255)
		local guntype = get_weapon_class(returngunid(builder_entity))
...
local function SnapLines()
	...
	drawxy = {}
	dronetable = {}
	-- if not linemaster:get() or not gui.GetValue("esp.master") then return end
	if not linemaster:get() then return end
	if plocallive then
		if hsmaster:get() then hss = entity_list.get_entities_by_name("CHostage") end
		if boxmaster:get() then lcs = entity_list.get_entities_by_name("CPhysPropLootCrate") end
		if ammomaster:get() then ams = entity_list.get_entities_by_name("CPhysPropAmmoBox") end
		if cashmaster:get() then chs = entity_list.get_entities_by_name("CItemCash") end
		if healmaster:get() then heals = entity_list.get_entities_by_name("CItem_Healthshot") end
		if endcirclemaster:get() then DZ = entity_list.get_entities_by_name("CDangerZoneController") end
		if dronemaster:get() then Drones = entity_list.get_entities_by_name("CDrone") end


		if linesubmaster:get() then
			if hss ~= nil and hsmaster:get() then
				for i = 1, #hss do
					local hs = hss[i];
					if hs:get_render_origin() ~= nil then
						local distance = (hs:get_render_origin() - localabs):Length()
						if distance < 6000 then
							local x, y = w2sreturnxy(hs:get_render_origin());
			...
			if lcs ~= nil and boxmaster:get() then
				for i = 1, #lcs do
					local lc = lcs[i];
					if lc:get_render_origin() ~= nil then
						local distance = (lc:get_render_origin() - localabs):Length()
						if distance < 3000 then
							local x, y = w2sreturnxy(lc:get_render_origin());
							if x ~= nil and y ~= nil then
			...
			if ams ~= nil and ammomaster:get() then
				for i = 1, #ams do
					local am = ams[i];
					if am:get_render_origin() ~= nil then
						local distance = (am:get_render_origin() - localabs):Length()
						if distance < 3000 then
							local x, y = w2sreturnxy(am:get_render_origin());
							if x ~= nil and y ~= nil then
			...
			if chs ~= nil and cashmaster:get() then
				for i = 1, #chs do
					local ch = chs[i];
					if ch:get_render_origin() ~= nil then
						local distance = (ch:get_render_origin() - localabs):Length()
						if distance < 3000 then
							local x, y = w2sreturnxy(ch:get_render_origin());
							if x ~= nil and y ~= nil then
			...
			if heals ~= nil and healmaster:get() then
				for i = 1, #heals do
					local heal = heals[i];
					if heal:get_render_origin() ~= nil then
						local distance = (heal:get_render_origin() - localabs):Length()
						if distance < 3000 and distance ~= 0 then
							local x, y = w2sreturnxy(heal:get_render_origin());
							if x ~= nil and y ~= nil then
		...
		if DZ ~= nil and endcirclemaster:get() then
			for i = 1, #DZ do
				local DZS = DZ[i]
				if DZS ~= nil then
					if DZS:get_prop("m_bDangerZoneControllerEnabled") == 1 then
						local startCirclePos = DZS:get_prop("m_vecEndGameCircleStart")
						if startCirclePos ~= nil then
							local x, y = w2sreturnxy(startCirclePos)
							if x ~= nil and y ~= nil then
		...
		if Drones ~= nil and dronemaster:get() then
			for i, Drone in pairs(Drones) do
				if Drone:get_render_origin() ~= nil then
					local distance = (Drone:get_render_origin() - localabs):Length()
					if distance < 6000 then
						local dx, dy = w2sreturnxy(Drone:get_render_origin());
						if dx ~= nil and dy ~= nil then
							if Drone:get_prop("m_hCurrentPilot") ~= -1 then
								table.insert(dronetable, { dx, dy, "Manual Controlled" })
								if distance < BestMDistance then
									BestMDistance = distance
								end
							end
							if Drone:get_prop("m_hDeliveryCargo") ~= -1 then
				...
				if Enemies ~= nil then
					...
					for i, Enemy in pairs(Enemies) do
						if Enemy:IsAlive() then
							local velocity = 0
							local cvx = Enemy:get_prop('localdata', 'm_vecVelocity[0]')
							local cvy = Enemy:get_prop('localdata', 'm_vecVelocity[1]')
							if cvx ~= nil then
								velocity = math.floor(math.min(10000, math.sqrt(cvx * cvx + cvy * cvy) + 0.5))
							end
							if get_weapon_class(returngunid(Enemy)) == "Tablet" and velocity < 100 then
...
local function drawEspHook(builder)
	...
	if plocallive then
		...
		if builder_entity:get_render_origin() == nil then return end
		local Distance = (localabs - builder_entity:get_render_origin()):Length()

		if builder_name == "weapon_shield" or builder_name == "armor and helmet" then
			if linesubmaster:get() and Distance < 3000 and Distance ~= 0 and shieldarmormaster:get() then
				local x, y = w2sreturnxy(builder_entity:get_render_origin());
				if x ~= nil and y ~= nil then
					table.insert(drawespxy, { x, y, screenCenterX - 800, screenH, 0, 0, 0, 255 })
				end
			end
			if Distance < 5000 and Distance ~= 0 and guidemaster:get() then
		...
		if builder_name == "weapon_glock" or builder_name == "weapon_hkp2000" or builder_name == "pistol" or builder_name == "light weapons" or builder_name == "random drop" or builder_name == "cashbag" then
			if linesubmaster:get() and Distance < 3000 and Distance ~= 0 and wpmaster:get() then
				local x, y = w2sreturnxy(builder_entity:get_render_origin());
				if x ~= nil and y ~= nil then
		...
		for _, key in pairs(drawespxy) do
			...
			render.line(vec2_t(key[1], key[2]), vec2_t(key[3], key[4]), color_t(key[5], key[6], key[7], key[8]))
		end
	end
end
...
local function DrawLine()
	if not linemaster:get() or not gui.GetValue("esp.master") then return end
	if linesubmaster:get() and ingame() then
		-- draw.Color(255, 255, 255, 255);
		-- draw.SetFont(font1);
		render.text(font1, "SubSnapLine", vec2_t(screenCenterX - 782, screenH / 2 - 100), color_t(255, 255, 255, 255))
	end
	...
	for _, key in pairs(dronetable) do
		if BestMDistance < 1200 then
			...
			render.text(fontA, key[3], vec2_t(key[1], key[2]), color_t(255, 0, 0, 255))
		else
			render.text(font, key[3], vec2_t(key[1], key[2]), color_t(255, 255, 255, 255))
		end
		-- draw.Text(font,key[3],vec2_t(key[1], key[2]),color_t(255, 255, 255, 255))
	end
	if #dronetable ~= 0 and BestMDistance < 6000 then
		if BestMDistance < 1200 then
			render.text(fontA, "MD Distance:" .. math.floor(BestMDistance),
				vec2_t(screenCenterX - 800, screenH / 2 + 200),
				color_t(255, 0, 0, 255));
		else
			render.text(font, "MD Distance:" .. math.floor(BestMDistance), vec2_t(screenCenterX - 800, screenH / 2 + 200),
	...
	if ENDdistance ~= 0 then
		draw.Text(font,
			"EndCircle Distance:" .. ENDdistance, vec2_t(screenCenterX - 800, screenH / 2 + 140), color_t(255, 0, 0, 255));
	end
end
...
callbacks.Register("CreateMove", function()
	...
	if pLocal == nil then
		...
		Enemies = entity_list.get_entities_by_name("CCSPlayer")
		if pLocal:IsAlive() then
			isNeedW = true
			if weaponstr ~= "weapon_fists " and ingame() then
				plocallive = true
				localabs = pLocal:get_render_origin()
				localweaponid = returngunid(pLocal)
				localarmor = pLocal:get_prop("m_ArmorValue")
			else
				plocallive = false
			end
		else
...
local function RemoteBombDetector()
	if plocallive and remotebombmaster:get() then
		rbvec = {}
		local rbs = entity_list.get_entities_by_name("CBreachChargeProjectile")
		if rbs == nil then return end
		for i = 1, #rbs do
			local rb = rbs[i]
			local rbabs = rb:get_prop("m_vecOrigin")
			local Distance = (localabs - rbabs):Length()

			if Distance < 1599 then
				local x, y = w2sreturnxy(rbabs)
				if x ~= nil and y ~= nil then
					table.insert(rbvec, { x, y, rb:get_prop("m_bShouldExplode") == 1 })
				end
			end
		end
	end
...
local function BarrelsDetector()
	if plocallive and barrelmaster:get() then
		bavec = {}
		local barrels = entity_list.get_entities_by_name("CPhysicsProp")
		if barrels == nil then return end
		for i = 1, #barrels do
			local barrel = barrels[i]
			local baabs = barrel:get_prop("m_vecOrigin")
			local modelid = barrel:get_prop("m_nModelIndex")
			...
			if modelid == 1039 or modelid == 1448 or modelid == 1079 or modelid == 996 then
				if Distance < 1599 then
					...
					local x, y = w2sreturnxy(baabs + Vector3(0, 0, maxvec.z))
					if x ~= nil and y ~= nil then
...
callbacks.Register("FireGameEvent", function(e)
	...
	elseif eventName == "round_prestart" or eventName == "round_poststart" or eventName == "round_start" then
		...
		if mapglassplace[map_name] then
			materials.Find(mapglassplace[map_name]):SetMaterialVarFlag(4, removegrassmaster:get())
		end
	elseif eventName == "player_death" and ingame() then
		local teamid = (entities.GetByUserID(e:GetInt("userid"))):get_propInt("m_nSurvivalTeam")
