--This source code comes from ZGXAimware and was later modified by HuYiDao(github.com/huyudai) to the final usable version of pri
--Last Update:2025/10/5 Version:1.0.0 Stable
local panorama = require("panorama library.lua")
local ui = {}
ui.group_name = "DangerZone Elite Sniffer"
ui.messagemode = menu.add_selection(ui.group_name, "SendWay", { "Only Console", "Global Chat", "Team Chat" })
...
ui.exitmaster = menu.add_checkbox(ui.group_name, "Exit sniffer", true)
...
local function partyapisay(message)
    ...
    elseif ui.messagemode:get() == 3 then
        engine.execute_cmd(('say_team "%s"'):format(message))  
    end
end
